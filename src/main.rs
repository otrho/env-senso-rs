...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut temp: f32 = (air_data[0] & TEMP_VALUE_MASK) as f32 + (air_data[1] as f32 / 10.0);
    if air_data[0] & TEMP_SIGN_MASK != 0 {
        temp = -temp;
    }
    let pressure = decode_i32(2);
    let humidity: f32 = air_data[6] as f32 + (air_data[1] as f32 / 10.0);
    let gas = decode_i32(8);

    println!("Temperature: {:.1} 'C", temp);
    println!("Pressure: {} hPa", pressure as f32 / 100.0);
    println!("Humidity: {}%", humidity);
    println!("Gas: {} Ohms", gas);
